...
TEST_CASE("naive", "sycl_matrix_transpose") {
     ...
     cppcon::benchmark(
         [&]() {
            defaultQueue.submit([&](cl::sycl::handler &cgh) {
              auto inputMatAcc =
                 inputMatBuf.template get_access<cl::sycl::access::mode::read>(cgh);
              auto outputMatAcc =
                 outputMatBuf.template get_access<cl::sycl::access::mode::write>(cgh);
...
TEST_CASE("local_mem", "sycl_matrix_transpose") {
   auto inputMat = matrix<float, WIDTH, HEIGHT>{};
   auto outputMat = matrix<float, WIDTH, HEIGHT>{};
             ...
             defaultQueue.submit([&](cl::sycl::handler &cgh) {
                ...
                auto scratchpad =
                  cl::sycl::accessor<float, 1, cl::sycl::access::mode::read_write,
                        ...
                        cl::sycl::range<1>(WORK_GROUP_WIDTH * WORK_GROUP_HEIGHT), cgh);
                ...
                cgh.parallel_for<local_mem>(
                   ...
                   [=](cl::sycl::nd_item<1> item) {
                      auto columnMajorId =
                          (item.get_global_id(1) * item.get_local_range(0)) +
